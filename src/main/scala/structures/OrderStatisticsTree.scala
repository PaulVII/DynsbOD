...
// an order statistics tree as decribed here https://en.wikipedia.org/wiki/Order_statistic_tree
// using the scala std library red-black tree as base
class ContainedTuplesSet:
  private val tree = RedBlackTreeWithSizes.Tree.empty[Int, Null]

  def add(id: Int): Unit =
    if !RedBlackTreeWithSizes.contains(tree, id) then
      RedBlackTreeWithSizes.insert(tree, id, null)

  def remove(id: Int): Unit =
    if RedBlackTreeWithSizes.contains(tree, id) then
      RedBlackTreeWithSizes.delete(tree, id)

  def contains(id: Int): Boolean = RedBlackTreeWithSizes.contains(tree, id)

  /** Number of contained IDs strictly greater than `id` */
  def countGreaterThan(id: Int): Int =
    tree.size - RedBlackTreeWithSizes.rank(tree, id)

  /** Iterator over all contained IDs strictly greater than `id` */
  def iteratorGreaterThan(id: Int): Iterator[Int] =
    RedBlackTreeWithSizes.keysIterator(tree, Some(id + 1))

  def iterator: Iterator[Int] =
    RedBlackTreeWithSizes.keysIterator(tree)
...
/** An object containing the red-black tree implementation used by mutable
  * extended to keep track of the size of nodes `TreeMaps`.
  *
  ...
  * @since 2.12
  */
private object RedBlackTreeWithSizes {
  ...
  final class Node[A, B](
    ...
    override def toString: String =
      "Node(" + key + ", " + value + ", " + red + ", " + left + ", " + right + ", " + size + ")"
  }
  ...
  object Node {
    ...
    @inline def apply[A, B](
      ...
      new Node(
        ...
        parent,
        // added
        1 + (if (left eq null) 0 else left.size) + (if (right eq null) 0
                                                    else right.size)
      )
    ...
    @inline def leaf[A, B](
      ...
      new Node(key, value, red, null, null, parent, 1)
  ...
  def size(node: Node[_, _]): Int =
    // changed
    if (node eq null) 0 else node.size
  ...
  // rank added
  /** Returns the number of nodes with key <= `key` (works even if key not
    * present).
    */
  def rank[A, B](tree: Tree[A, B], key: A)(implicit ord: Ordering[A]): Int =
    var x = tree.root
    var r = 0
    while (x ne null) {
  ...
  /** Returns the first (lowest) map entry with a key equal or greater than
    * `key`. Returns `None` if there is no such node.
    */
  ...
  /** Returns the last (highest) map entry with a key smaller than `key`.
  ...
  def insert[A, B](tree: Tree[A, B], key: A, value: B)(implicit
    ...
    if (cmp == 0) y.value = value
      ...
      tree.size += 1
      // added:
      // recompute sizes up from the inserted node to ensure subtree sizes are correct
      recomputeSizesUpFrom(z)
    }
  }
  ...
  def delete[A, B](tree: Tree[A, B], key: A)(implicit
    ...
    if (z ne null) {
      ...
      tree.size -= 1
      // added:
      // ensure sizes are consistent after deletion
      if (xParent ne null) recomputeSizesUpFrom(xParent)
      else if (tree.root ne null) recomputeSizesUpFrom(tree.root)
    }
  }
  ...
  // added:
  private[this] def recomputeSizesUpFrom[A, B](node: Node[A, B]): Unit =
    var n = node
    while (n ne null) {
  ...
  /** Returns the node that follows `node` in an in-order tree traversal. If
    * `node` has the maximum key (and is, therefore, the last node), this method
    * returns `null`.
    */
  ...
  /** Returns the node that precedes `node` in an in-order tree traversal. If
    * `node` has the minimum key (and is, therefore, the first node), this
    * method returns `null`.
    */
  ...
  private[this] def rotateLeft[A, B](tree: Tree[A, B], x: Node[A, B]): Unit =
    if (x ne null) {
      ...
      y.left = x
      x.parent = y
      // update sizes: x now has new children, y becomes parent of x
  ...
  /** Transplant the node `from` to the place of node `to`. This is done by
    * setting `from` as a child of `to`'s previous parent and setting `from`'s
    * parent to the `to`'s previous parent. The children of `from` are left
    * unchanged.
    */
  ...
  private[this] abstract class TreeIterator[A, B, R](
    ...
    private[this] def setNullIfAfterEnd(): Unit =
      if (
        end.isDefined && (nextNode ne null) && ord.compare(
          nextNode.key,
          end.get
        ) >= 0
      )
  ...
  private[this] def hasProperParentRefs[A, B](tree: Tree[A, B]): Boolean = {
    ...
    def hasProperParentRefs(node: Node[A, B]): Boolean = {
      if (node eq null) true
        ...
        if (
          (node.left ne null) && (node.left.parent ne node) ||
  ...
  /** Returns true if this node follows the properties of a binary search tree.
    */
  private[this] def isValidBST[A, B](
      node: Node[A, B]
  )(implicit ord: Ordering[A]): Boolean = {
    if (node eq null) true
    else {
      if (
        (node.left ne null) && (ord.compare(node.key, node.left.key) <= 0) ||
  ...
  /** Returns true if the tree has all the red-black tree properties: if the
    * root node is black, if all children of red nodes are black and if the path
    * from any node to any of its null children has the same number of black
    * nodes.
    */
